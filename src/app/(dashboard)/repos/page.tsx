...
/**
 * Página para gestionar los repositorios de un usuario.
 *
 * Esta página permite a los usuarios ver los repositorios que ya han conectado a la aplicación,
 * buscar y conectar nuevos repositorios desde su cuenta de GitHub, y desconectar
 * repositorios existentes.
 *
 * Funcionalidades:
 * - Lista de repositorios conectados desde la base de datos local.
 * - Opción para mostrar y buscar repositorios desde la API de GitHub.
 * - Selección múltiple para conectar varios repositorios a la vez.
 * - Desconexión individual de repositorios.
 * - Indicadores visuales para el estado de carga y errores.
 */
export default function ReposPage() {
  ...
  const connectedRepos = trpc.repository.list.useQuery();                    // Consulta los repositorios ya vinculados desde la BD.
  const githubRepos = trpc.repository.fetchFromGithub.useQuery(undefined, {  // Consulta todos los repositorios del usuario desde GitHub.
    enabled: showGitHubRepos,
  });

  const connectMutation = trpc.repository.connect.useMutation({              // Define la mutación para vincular repositorios en la BD.
    onSuccess: () => {
  ...
  const disconnectMutation = trpc.repository.disconnect.useMutation({        // Define la mutación para desvincular un repositorio.
    onSuccess: () => {
      connectedRepos.refetch();
    },
  });

  const connectedIds = new Set(                                              // Crea un Set con los IDs de los repos ya conectados para búsquedas rápidas.
    connectedRepos.data?.map((repo) => repo.githubId) || [],
  );

  const availableRepos =                                                     // Filtra la lista de GitHub para mostrar solo los que no están conectados.
    githubRepos.data?.filter((repo) => !connectedIds.has(repo.githubId)) || [];

  const filteredAvailableRepos = availableRepos.filter(                      // Aplica el filtro de búsqueda sobre los repositorios disponibles.
    (repo) =>
      repo.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      repo.description?.toLowerCase().includes(searchQuery.toLowerCase()),
  );

  const toggleRepo = (githubId: number) => {                                 // Gestiona la selección/deselección de un repo para vincularlo.
  ...
  const handleConnect = () => {                                             // Ejecuta la mutación para conectar los repositorios seleccionados.
  ...
  const selectAll = () => {                                               // Selecciona todos los repositorios filtrados y disponibles.
    setSelectedRepos(new Set(filteredAvailableRepos.map((r) => r.githubId)));
  };

  const clearSelection = () => {                                          // Limpia la selección de repositorios.
    setSelectedRepos(new Set());
  };

  return (
    <div className="space-y-8">
      {/* --- Encabezado Principal: Título y Botón "Add Repository" --- */}
      ...
      {/* --- Panel de Importación (Visible solo al pulsar "Add Repository") --- */}
      {showGitHubRepos && (
        <Card className="overflow-hidden">
          ...
          <CardContent className="p-0">
            {/* Estado: Cargando, Error, Vacío o Lista de Resultados */}
            {githubRepos.isLoading ? (
              ...
              <>
                ...
                {/* Pie del Panel: Contador y Botón de Acción "Connect" */}
      ...
      {/* --- Sección de Repositorios Conectados (Lista Principal) --- */}
...
/**
 * Tarjeta individual para un repositorio ya conectado.
 * Muestra información básica, estado (privado/público) y acciones:
 * - Ver PRs (Link)
 * - Desconectar (AlertDialog para confirmar borrado)
 */
...
/**
 * Fila seleccionable en la lista de importación de GitHub.
 * Incluye checkbox, nombre, descripción, estrellas y lenguaje.
 */
...
/**
 * Formatea una fecha de manera relativa (ej: "2 days ago")
 * o absoluta si es muy antigua.
 */
