...
type ReviewStatus = "all" | "COMPLETED" | "PROCESSING" | "PENDING" | "FAILED";  // Define los estados posibles para el filtrado de revisiones.

export default function ReviewsPage() {

  const [statusFilter, setStatusFilter] = useState<ReviewStatus>("all");        // Estado para controlar el filtro activo de las revisiones (por defecto 'all').

  // Consulta a la API (tRPC) para obtener la lista de revisiones.
  // Incluye lógica de "polling" (refresco automático) si hay revisiones en curso.
  const reviews = trpc.review.list.useQuery(
    { limit: 50 },
    {
      refetchInterval: (query) => {
        ...
        return hasProcessing ? 3000 : false; // Refresca cada 3s si hay procesos activos.
      },
    },
  );

  // Mutación para reintentar una revisión fallida manualmente.
  const triggerReview = trpc.review.trigger.useMutation({
    onSuccess: () => {
      reviews.refetch(); // Recarga la lista al tener éxito.
    },
  });

  // Filtra las revisiones en el cliente según el estado seleccionado.
  const filteredReviews = reviews.data?.filter(
    (r) => statusFilter === "all" || r.status === statusFilter,
  );

  // Calcula los contadores para cada estado para mostrarlos en las pestañas.
  const statusCounts = {
  ...
  return (
    <div className="space-y-8">
      {/* Encabezado de la página con título y botón de refresco manual */}
      ...
      {/* Barra de pestañas para filtrar las revisiones por estado */}
      ...
      {/* Renderizado condicional: Carga (Skeleton), Error, Vacío o Lista de tarjetas */}
      {reviews.isLoading ? (
        <div>
          {[...Array(5)].map((_, i) => (
        ...
        <div className="space-y-3">
          {/* Mapeo de las revisiones filtradas a tarjetas individuales */}
...
/**
 * Componente tarjeta que muestra los detalles de una revisión individual.
 * Incluye estado, enlace al PR, puntuación de riesgo y acciones.
 */
function ReviewCard({ review, onRetry }: ReviewCardProps) {
  ...
  // Genera un mensaje descriptivo basado en el estado actual.
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-4 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1 space-y-1">
              ...
              {/* Muestra detalles adicionales solo si la revisión está completada */}
          ...
          {/* Acciones: Enlace externo al PR y botón de acción (Retry/View) */}
...
// Devuelve la clase de color de fondo según el estado.
...
// Componente Badge para mostrar el estado en texto.
...
// Componente Icono que varía según el estado.
function StatusIcon({
  status,
  className,
}: {
...
// Componente para visualizar la puntuación de riesgo con código de colores.
...
// Configuración de colores y etiquetas para los niveles de riesgo.
...
// Formatea la fecha a un formato relativo legible (ej. "2h ago").
